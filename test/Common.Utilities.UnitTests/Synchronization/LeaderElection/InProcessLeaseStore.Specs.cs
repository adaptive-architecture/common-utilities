...
public class InProcessLeaseStoreSpecs
{
    ...
    public async Task TryRenewLeaseAsync_WithDifferentParticipant_ShouldReturnNull()
    {
        ...
        // Act
        await store.TryAcquireLeaseAsync(
            DefaultElectionName,
            DefaultParticipantId,
            _defaultLeaseDuration);
            ...
            .Select(i => Task.Run(async () =>
            {
                ...
                for (int j = 0; j < iterationCount; j++)
                {
                    ...
                    if (lease != null)
                    {
                        ...
                        await store.ReleaseLeaseAsync(electionName, participantId);
                    }
                }
    ...
    public async Task ExpiredLeaseCleanup_ShouldRemoveExpiredEntries()
    {
        ...
        // Act
        await store.TryAcquireLeaseAsync(DefaultElectionName, DefaultParticipantId, _defaultLeaseDuration);
