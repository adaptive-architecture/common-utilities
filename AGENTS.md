...
This file provides technical guidance for AI coding agents working with the Common Utilities repository. It complements README.md by containing detailed development practices and architectural constraints.
...
dotnet test --filter "FullyQualifiedName!~AdaptArch.Common.Utilities.Samples"  # Exclude samples
```
...
- Extensions: DateTime, Dictionary, Task, JSON, Exception
- Synchronization: ExclusiveAccess for thread-safe operations

**Specialized Modules**:
- `AspNetCore`: HttpContext extensions
- `Redis`: Message hub (pub/sub), leader election, lease store
...
**Constitution Version**: 1.0.0 | **Ratified**: 2025-09-19 | **Last Amended**: 2025-09-19
...
1. Create project structure:
   - `src/AdaptArch.Common.Utilities.NewModule/`
   - `test/AdaptArch.Common.Utilities.NewModule.Tests/`
   - `samples/AdaptArch.Common.Utilities.NewModule.Samples/`

2. Add `InternalsVisibleTo` attribute in source project for test access

3. Follow TDD: write tests first, then implement

4. Ensure NuGet package configuration is complete
